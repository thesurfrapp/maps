# [Maps] Wind stations — two-phase loading (live-cluster paint + live hydration) in windy-stations.ts
...
cached static live-cluster (positions + pre-baked cluster nodes; zero network on repeat
...
- **Version guard:** readings `v` ≠ live-cluster `v` → refetch live-cluster (usually a 304).
...
- **bbox endpoint leaves the map path.** `/windystations/bbox` remains only for
  search, tap details (names live there — the live-cluster has none), and legacy app
  versions.
...
- `src/lib/windy-stations.ts` — main rework: source setup (URL-fed live-cluster), node +
...
- [ ] Repeat map open paints nodes/stations with zero station-related network
      requests (HTTP cache); cold open needs exactly two (live-cluster + readings)
...
- Inactive-stations filter UI and the all-variant live-cluster
