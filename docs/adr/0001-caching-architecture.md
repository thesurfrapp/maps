...
`GET https://surfr-tile-warmer-cron.herbert-0fd.workers.dev/force?domain=<d>&token=<ADMIN_TOKEN>`
...
`https://maps.thesurfr.app/tiles/_admin?token=<ADMIN_TOKEN>` — HTML page
...
Related endpoints (all require the `ADMIN_TOKEN` shared secret, passed as
`Authorization: Bearer <t>` or `&token=<t>`; tile-serving paths stay public):
...
The secret is stored as a Pages secret and a worker-cron Worker secret
(same value): `npx wrangler pages secret put ADMIN_TOKEN --project-name maps`
(then redeploy Pages) and `cd worker-cron && npx wrangler secret put
ADMIN_TOKEN` (then redeploy the worker). The cron worker authenticates its
calls to `_warmer-trigger` with the same token. Endpoints fail closed when
the secret is unset.
